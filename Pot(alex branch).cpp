...
\param potPin the Arduino analog pin number. Default is A0.
*/
...
void PotClass::setMax() {
	maxAngle = getRawValue();    // read the value from the sensor
}
/*!
...
{
	return "raw:[" + String(getRawValue()) + "] Angle:[" + String(getAngle()) + "] Min:[" + String(minVal) + "] Max:[" + String(maxVal) + "]" + "] MinAngle:[" + String(minAngle) + "] MaxAngle:[" + String(maxAngle) + "]";
}

//CHANGES!!!!!!!!!!!!
/*!
  gets the height of the elbow
 */
float PotClass::angleinRads(){
   return (getAngle()*(pi/180));
}

//float PotClass::rad2Deg(){
//  return 
//}
 
float PotClass::getElbowHeight(){
   heightfromParallel = (armLength*sin(angleinRads()));
   elbowHeight = (shoulderHeight + heightfromParallel);
   return elbowHeight;
 }
